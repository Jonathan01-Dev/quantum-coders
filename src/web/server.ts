...
export class WebDashboard {
    ...
    private setupExpress() {
        ...
        this.app.get('/api/dht', (req, res) => {
            res.json(this.node.getDht());
        });
        ...
        this.app.post('/api/ai', async (req, res) => {
            try {
                ...
                console.log(`[HTTP] AI request received: "${prompt?.slice(0, 30)}..."`);
                const response = await this.node.askGeminiLocal(prompt);
                res.json({ response });
            } catch (e) {
                console.error(`[HTTP ERROR] /api/ai:`, e);
                res.status(500).json({ error: (e as Error).message });
            }
        });
        ...
        this.app.post('/api/summarize', async (req, res) => {
            try {
                ...
                console.log(`[HTTP] Summarize request for ${manifestId}`);
                const response = await this.node.summarizeFile(manifestId);
                res.json({ response });
            } catch (e) {
                console.error(`[HTTP ERROR] /api/summarize:`, e);
                res.status(500).json({ error: (e as Error).message });
            }
        });
    }
    ...
    private setupWebSocket() {
        this.wss.on('connection', (ws) => {
            ...
            ws.send(JSON.stringify({
                ...
                dht: this.node.getDht(),
                stats: this.node.getNodeStats(),
                logs: this.logs
            }));
    ...
    private hookNodeEvents() {
        ...
        this.node.on('file:shared', (data: any) => {
            this.broadcast('LOG', `[SHARED] File ${data.filename} published! Manifest ID: ${data.file_id}`);
        });

        this.node.on('dht:update', (data: any) => {
            this.broadcast('DHT_UPDATE', data);
        });

        this.node.on('transfer:progress', (data: any) => {
            this.broadcast('TRANSFER_PROGRESS', data);
        });
    }
}
